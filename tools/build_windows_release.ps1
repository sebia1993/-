...
chcp 65001 >nul
cd /d "%~dp0"
echo TCP 전송 성능 측정 클라이언트를 시작합니다.
...
TCP 전송 성능 측정:
1. TCP 전송 성능 측정은 기본으로 함께 시작됩니다.
2. 콘솔에 표시된 실제 TCP 포트를 확인하고, 다른 PC에서 연결되지 않으면 안내된 방화벽 명령을 실행합니다.
3. 웹 화면의 TCP 전송 성능 측정에서 Windows 클라이언트 ZIP을 받습니다.
...
- TCP 전송 성능 측정 포트가 사용 중이면 빈 포트를 제안하고 승인 후 저장합니다.

주의:
- 코드서명하지 않은 EXE이므로 Windows SmartScreen 경고가 표시될 수 있습니다.
- 실제 업로드 파일과 운영 CSV·JSON 기록은 GitHub에 올리지 마세요.
- TCP 전송 성능 측정은 Windows 클라이언트용이며 Android에서는 웹 HTTP 측정만 지원합니다.
...
- HTTP 연결 1개로 3초 워밍 후 10초/30초 측정, 1초 구간 그래프, HTTP 응답시간, 취소 지원
...
- 실제 수신 평균·MB/s·중앙값·최소·최대, RTT와 재전송 비율 중심의 TCP 웹 요약
...
- DB를 사용하지 않습니다.
- HTTP 데이터량·측정 시간 기준은 브라우저 HTTP 전송 성능이며 TCP·UDP 전송 성능 측정이나 iperf 결과와는 다릅니다.
- TCP 전송 성능 측정은 자체 TCP 프로토콜이며 iperf 실행파일·라이브러리·호환 프로토콜을 사용하지 않습니다.
