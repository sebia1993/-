...
1. `internal-upload_v0.4.1_windows.zip`을 다운로드합니다.
...
현재 정식 Release asset은 `internal-upload_v0.4.1_windows.zip`과 SHA256 파일입니다. `v0.4.1`은 `v0.4.0`의 기능과 저장 형식을 유지하면서 원자적 파일 저장, TCP 자원 상한, CSV 동시성과 측정 결과 내구성을 보강한 유지보수 릴리즈입니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
