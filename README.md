...
1. `internal-upload_v0.4.0-rc.6_windows.zip`을 다운로드합니다.
...
- `network_probe.ENABLED`: Windows TCP 정밀 측정 서버를 켭니다. 기본값은 `true`이며 필요하면 `false`로 끌 수 있습니다.
...
TCP 정밀 측정 포트가 사용 중이면 다음 99개 포트에서 빈 포트를 찾아 변경 여부를 묻습니다. 승인한 포트는 TCP 서버 바인딩 성공 후 `[network_probe] PORT`에 저장합니다. 변경을 거절하거나 사용할 포트가 없으면 파일 업로드 웹 서버와 HTTP 측정은 계속 실행하고 TCP 정밀 측정만 사용할 수 없게 표시합니다. 이전 릴리즈의 `ENABLED=false` 설정은 `rc.6` 첫 실행에서 한 번 `true`로 전환되며, 이후 사용자가 다시 `false`로 설정하면 그대로 유지됩니다.
...
1. `start_internal_upload.cmd`를 실행합니다. TCP 정밀 측정은 기본으로 함께 시작됩니다.
...
2. `TCP 정밀 측정`에서 `Windows 클라이언트 ZIP 받기`를 누릅니다.
...
HTTP 용량 기준·시간 기준 측정은 브라우저와 Flask 서버 사이의 HTTP 응용 전송 성능입니다. TCP 정밀 측정은 브라우저를 데이터 경로에서 제외하지만 자체 프로토콜이므로 iperf 클라이언트·서버와 호환되지 않습니다. 모든 측정값에는 단말 CPU, NIC, Wi-Fi와 서버 PC 성능이 함께 반영됩니다. 설계 대상은 1Gbps 이하 사내망입니다.
...
HTTP 용량 기준 측정은 `data/network_check_log.csv`에 저장됩니다. HTTP 시간 기준 측정 요약은 `data/network_check_session_log.csv`, 상세 원본은 `data/network_check_results/<session_id>.json`에 저장됩니다. 화면의 Excel 파일은 이 원본에서 요청할 때 메모리로 생성하므로 서버에 별도 저장하지 않습니다. TCP 정밀 측정은 `data/network_probe_log.csv`와 `data/network_probe_results/<session_id>.json`에 저장됩니다. 운영 CSV와 JSON은 GitHub에 올리지 마세요.
...
현재 시험용 Release asset은 `internal-upload_v0.4.0-rc.6_windows.zip`과 SHA256 파일입니다. `rc.6`는 HTTP 측정 명칭을 기준별로 정리하고 TCP 정밀 측정과 웹 클라이언트 ZIP을 기본 활성화한 사전 릴리즈입니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
