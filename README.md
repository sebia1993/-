...
1. `internal-upload_v0.4.0-rc.8_windows.zip`을 다운로드합니다.
...
현재 시험용 Release asset은 `internal-upload_v0.4.0-rc.8_windows.zip`과 SHA256 파일입니다. `rc.8`은 TCP 결과 화면과 Excel 첫 시트를 네트워크 초급 관리자와 보고 대상자가 이해하기 쉬운 실제 수신 속도 중심으로 정리한 사전 릴리즈입니다. 기존 측정 엔진, API, CSV와 JSON 형식은 유지합니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
