...
1. `internal-upload_v0.2.2_windows.zip`을 다운로드합니다.
...
- 측정 방향: 업로드, 다운로드, 전체 측정
- 테스트 크기: `10MB`, `50MB`, `100MB`, `500MB`, `1024MB`
- 측정 결과: 진행률, 평균 속도, 구간 속도, 완료 속도
- 업로드 진행률은 서버가 테스트 데이터를 받은 조각 기준으로 갱신됩니다.
- 속도는 `Mbps`와 `MB/s`를 함께 표시합니다.
- 측정 중 취소할 수 있으며, `1024MB` 측정은 시작 전에 확인창이 표시됩니다.

이 기능은 인터넷 회선 속도 측정이 아닙니다. 사내망에서 현재 브라우저가 접속한 PC와 사내 업로드 서버 PC 사이의 전송 속도를 확인하는 용도입니다.
...
네트워크 체크 기록은 `data/network_check_log.csv`에 별도로 저장됩니다. 기존 업로드 기록과 속도 측정 기록은 섞지 않습니다.
...
현재 사용자용 Release asset은 `internal-upload_v0.2.2_windows.zip`입니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
