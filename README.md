...
1. `internal-upload_v0.4.0-rc.4_windows.zip`을 다운로드합니다.
...
웹 포트 충돌로 포트 변경을 승인하면 `PORT`는 새 값으로 저장됩니다. `BASE_URL`이 기존 웹 포트를 사용하고 있을 때만 해당 포트도 함께 변경합니다. 별도 프록시 포트처럼 다른 포트를 사용 중인 `BASE_URL`은 유지하고 콘솔에 주의 문구를 표시합니다. 이미 같은 서버가 실행 중이면 새 서버를 중복 실행하지 않고 기존 주소만 안내합니다. TCP 정밀 측정 포트 `5201`은 이 과정에서 변경하지 않습니다.
...
클라이언트 ZIP은 접속 중인 서버 주소와 웹 포트를 자동으로 포함합니다. 서버 웹 화면을 `localhost` 또는 `127.0.0.1`로 열었다면 프로그램이 감지한 서버 PC의 사내 IPv4를 대신 사용합니다. 서버 IP 또는 웹 포트가 바뀌면 ZIP을 다시 받으세요. 클라이언트 ZIP에는 `config.ini`, 인증 토큰이나 세션 정보가 들어가지 않습니다.

웹 다운로드는 `[network_probe] ENABLED=true`이고 Windows Release EXE로 서버를 실행할 때만 사용할 수 있습니다. 소스 실행 환경이나 자동 주소 감지가 어려운 환경에서는 전체 Release ZIP의 `start_tcp_probe_client.cmd`를 수동 대체 수단으로 사용하고 서버 주소를 직접 입력합니다.
...
현재 시험용 Release asset은 `internal-upload_v0.4.0-rc.4_windows.zip`과 SHA256 파일입니다. `rc.4`는 HTTP 지속 측정 결과의 Excel 다운로드를 추가한 사전 릴리즈입니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
