...
1. `internal-upload_v0.4.0_windows.zip`을 다운로드합니다.
...
- 웹 요약에서 실제 수신 속도, RTT, TCP 재전송량과 전체 송신량 대비 비율, 측정 조건과 측정 PC 표시
...
현재 정식 Release asset은 `internal-upload_v0.4.0_windows.zip`과 SHA256 파일입니다. `v0.4.0`은 HTTP 전송 측정과 Windows TCP 전송 성능 측정, 자동 연결 클라이언트 ZIP, 관리자용 Excel 결과를 포함한 정식 릴리즈입니다. 기존 `v0.4.0-rc.8`의 측정 엔진, API, CSV와 JSON 형식을 그대로 유지합니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
