...
1. `internal-upload_v0.3.0_windows.zip`을 다운로드합니다.
...
이 기능은 브라우저와 Flask 서버 사이의 HTTP 응용 전송 성능을 측정합니다. TCP 재전송·CWND, UDP 손실률·지터를 측정하는 iperf 대체 기능이 아니며, 결과에 브라우저, 단말 CPU, Wi-Fi, 서버 PC 성능이 함께 반영됩니다. 설계 대상은 1Gbps 이하 사내망입니다.
...
간편 네트워크 체크는 `data/network_check_log.csv`에 저장됩니다. 지속 측정 요약은 `data/network_check_session_log.csv`, 1초 구간을 포함한 상세 결과는 `data/network_check_results/<session_id>.json`에 저장됩니다. 운영 CSV와 JSON은 GitHub에 올리지 마세요.
...
현재 사용자용 Release asset은 `internal-upload_v0.3.0_windows.zip`입니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
...
```bat
.venv\Scripts\python -m compileall app.py network_sustained.py tests tools
```

`v0.4.0` 자체 TCP 측정과 `v0.5.0` UDP 손실·지터 측정은 후속 계획이며 `v0.3.0`에는 포함되지 않습니다.
