...
1. `internal-upload_v0.4.0-rc.5_windows.zip`을 다운로드합니다.
...
현재 시험용 Release asset은 `internal-upload_v0.4.0-rc.5_windows.zip`과 SHA256 파일입니다. `rc.5`는 HTTP 지속 측정 진행률을 전체 작업 기준으로 개선한 사전 릴리즈입니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
