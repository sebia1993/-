...
1. `internal-upload_v0.4.3-rc.2_windows.zip`을 다운로드합니다.
...
서버와 클라이언트 릴리즈 버전은 웹 화면에 함께 표시됩니다. 측정 프로토콜이 같은 다른 릴리즈는 경고만 표시하고 사용할 수 있습니다. `v0.4.3-rc.2`는 `rc.1`과 같은 TCP 프로토콜 `v2`를 사용하므로 두 버전은 호환됩니다. `v0.4.2` 및 이전 클라이언트는 서버 웹 화면에서 최신 ZIP을 다시 받아야 합니다.
...
현재 정식 Release는 `v0.4.2`이며, 측정 그래프와 Excel 보고서를 검증하는 최신 사전 릴리즈 asset은 `internal-upload_v0.4.3-rc.2_windows.zip`과 SHA256 파일입니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
...
```bat
.venv\Scripts\python -m compileall app_version.py app.py startup_ports.py network_sustained.py sustained_excel.py excel_report.py network_measurement.py result_storage.py network_probe tests tools
```
