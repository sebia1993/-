...
1. `internal-upload_v0.4.6-rc.1_windows.zip`을 다운로드합니다.
...
서버와 클라이언트 릴리즈 버전은 웹 화면에 함께 표시됩니다. `v0.4.6-rc.1`은 TCP 프로토콜 `v2`를 유지하므로 `v0.4.3` 이후 클라이언트와 프로토콜상 호환되지만, 실행 역할이 분리된 최신 클라이언트 ZIP 사용을 권장합니다. `v0.4.2` 및 이전 클라이언트는 서버 웹 화면에서 최신 ZIP을 다시 받아야 합니다.
...
현재 정식 Release는 `v0.4.2`이며, 운영 안정성을 검증하는 최신 사전 릴리즈 asset은 `internal-upload_v0.4.6-rc.1_windows.zip`과 SHA256 파일입니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
