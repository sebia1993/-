...
1. `internal-upload_v0.4.2_windows.zip`을 다운로드합니다.
...
현재 정식 Release asset은 `internal-upload_v0.4.2_windows.zip`과 SHA256 파일입니다. `v0.4.2`는 측정 계산과 저장 형식을 유지하면서 HTTP·TCP 결과를 핵심 요약과 선택형 기술 상세로 정리한 화면 개선 릴리즈입니다. GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 소스 아카이브이며 일반 실행용 ZIP이 아닙니다.
