...
이 파일은 저장소에 커밋하는 릴리즈 준비 점검 문서입니다. 현재는 GitHub Release 자동 생성 workflow나 배포 ZIP asset을 만들지 않습니다. 버전 공유가 필요하면 Git tag와 GitHub Release를 수동으로 만들고, Release 본문은 이 파일과 `CHANGELOG.md` 기준으로 작성합니다.
...
현재 GitHub Release는 수동 생성합니다.

- 태그 형식: `v0.1.0`처럼 앱 버전과 맞춥니다.
- Release 제목: `v0.1.0 - 사내 업로드 초기 릴리즈`
- Release 본문: 포함 기능, 제외 항목, 검증 명령, 실행 방법, asset 정책을 한국어로 적습니다.
- 직접 업로드하는 Release asset은 없습니다.

GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 tag 기준 소스 아카이브입니다. 사용자가 실행할 별도 Windows EXE나 ZIP 파일은 아직 제공하지 않습니다.

향후 ZIP asset을 만들 경우, 아래 내용을 먼저 확정하고 이 파일을 갱신합니다.
...
- 고객명, 사이트명, 내부망 식별자
- 아직 구현하지 않은 로그인, 만료일, 관리자 페이지, EXE 배포, ZIP 배포 기능
