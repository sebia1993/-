...
- 태그 형식: 사전 릴리즈는 `v0.4.6-rc.2`, 정식 릴리즈는 `v0.4.2`처럼 관리합니다.
- Release 제목: `v0.4.6-rc.2 - 사내 업로드 Windows 안정성 개선`
- Release 본문: 포함 기능, 제외 항목, 검증 명령, 실행 방법, asset 정책을 한국어로 적습니다.
- 직접 업로드하는 Release asset: `internal-upload_v0.4.6-rc.2_windows.zip`, `.zip.sha256`
- SHA256 checksum은 별도 asset과 Release 본문에 기록합니다.
- 같은 태그의 GitHub Release가 이미 있으면 실패 처리하며 기존 asset을 덮어쓰지 않습니다.

GitHub가 자동으로 표시하는 `Source code (zip)` / `Source code (tar.gz)`는 tag 기준 소스 아카이브입니다. 이 사전 릴리즈 검증자는 `internal-upload_v0.4.6-rc.2_windows.zip`을 다운로드합니다.
...
python -m pytest -q
pwsh -NoProfile -File .\tools\build_windows_release.ps1 -Version v0.4.6-rc.2
dist\internal-upload_v0.4.6-rc.2_windows\InternalUploadServer.exe --smoke-check
dist\internal-upload_v0.4.6-rc.2_windows\InternalUploadServer.exe --probe-self-check
dist\internal-upload_v0.4.6-rc.2_windows\client-template\NetworkProbeClient.exe --self-check
python tools\verify_release_zip.py --zip dist\internal-upload_v0.4.6-rc.2_windows.zip --version v0.4.6-rc.2
```
